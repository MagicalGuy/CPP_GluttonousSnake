...
void Map::display() { //显示游戏面板int& score,int& level,int& healthPoint
	...
	for (int i = 0; i<20; i++)
	{
		...
		for (int j = 0; j<20; j++)
		{
			...
			case 2:utils.color(1); printf("☆"); break;   //食物
...
//	utils.gotoXY(46, 9); printf("SCORE: %d", score);      //记录分数
//	utils.gotoXY(46, 10); printf("LEVEL: %d", level); //记录等级
//	utils.gotoXY(46, 11); printf("HP: %d", healthPoint);
	utils.color(12);
	utils.gotoXY(46, 5); printf("C++贪吃蛇游戏！\n");
	utils.gotoXY(46, 6); printf("无聊解闷，你值得拥有！\n");
}
